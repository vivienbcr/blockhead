...
use crate::{conf2, prom::registry::track_response_time, prom::registry::track_status_code};
...
impl ReqwestClient {
    pub async fn rpc(
        ...
        for i in 0..self.config.retry.unwrap_or(conf2::DEFAULT_ENDPOINT_RETRY) {
            ...
            if response.is_err() {
                debug!(
                    ...
                    self.config
                        .rate
                        .unwrap_or(conf2::DEFAULT_ENDPOINT_REQUEST_RATE),
                    i,
                    self.config.retry.unwrap_or(conf2::DEFAULT_ENDPOINT_RETRY)
                );
                self.iddle().await;
                continue;
            }
            ...
            if status != 200 {
                debug!(
                    ...
                    self.config
                        .retry
                        .unwrap_or(conf2::DEFAULT_ENDPOINT_RETRY),
                    &b
                );
                self.iddle().await;
                continue;
        ...
        return Err(format!(
            ...
            url,
            self.config.rate.unwrap_or(conf2::DEFAULT_ENDPOINT_RETRY)
        )
        .into());
    }
