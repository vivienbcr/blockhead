...
use crate::{configuration::{ProtocolName,NetworkName, CONFIGURATION, get_enabled_protocol_network}, db::DATABASE, commons::blockchain::Blockchain};
...
async fn blockchain_handler(params: web::Query<BlockchainRouteParams>) -> HttpResponse {
    ...
    if params.protocol.is_some(){
        ...
        if !proto_net.contains_key(&param_protocol){
            return HttpResponse::BadRequest().body(format!("{} protocol not found",param_protocol.clone()));
        }
        if params.network.is_some(){
            ...
            if !proto_net.contains(&param_network){
                return HttpResponse::BadRequest().body(format!("{} network not found for {} protocol",param_network,param_protocol));
            }
        ...
        for network in available_networks{
            debug!("Getting Database {} network for {} protocol",network,param_protocol.clone());
            //FIXME: if database return one empty response, handler crash
            let response = db.get_blockchain(&param_protocol, &network).unwrap();
            data.entry(param_protocol.clone()).or_insert(HashMap::new()).insert(network,response);
        }
    }   
    ...
    for (protocol, networks) in proto_net{
        for network in networks{
            ...
            data.entry(protocol.clone()).or_insert(HashMap::new()).insert(network,response);
        }
    };
    HttpResponse::Ok()
        .content_type("application/json")
